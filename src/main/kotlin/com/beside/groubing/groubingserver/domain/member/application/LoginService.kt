...
class LoginService(
    ...
    fun login(loginCommand: LoginCommand): MemberLoginResponse {
        // 아이디가 존재하는지
        ...
        return MemberLoginResponse(member.id, member.email, JwtProvider.createToken(member.id, member.email, member.role))
    }
}
