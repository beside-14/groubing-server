...
@ApiTest
@WebMvcTest(controllers = [MemberFriendFindApi::class])
class MemberFriendFindApiTest(
    private val mockMvc: MockMvc,
    private val mapper: ObjectMapper,
    @MockkBean private val memberFriendFindService: MemberFriendFindService
) : BehaviorSpec({
    Given("유저가") {
        ...
        When("현재 연결된 친구 목록을") {
            ...
            every { memberFriendFindService.findById(any(), any()) } returns response

            Then("조회한다.") {
                mockMvc.perform(
                    get("/api/members/{inviterId}/friends", id)
        ...
        When("친구 목록에서 닉네임으로") {
            ...
            val response = PageImpl(listOf(friend.single()))
            every { memberFriendFindService.findByIdAndNickname(any(), any(), any()) } returns response
